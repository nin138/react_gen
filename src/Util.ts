...
export const Util = {
  ...
  enumToKeyArray: (enumObj: any): string[] => {
...
export const NodeUtil = {
  ...
  attrToString: (info: AttributeInfo, value: string): string => {
    if (info.isRequired == true && value == "") {
      throw new Error(`attribute "${info.name}" is required but has no value`);
    }
    const getValue = (attr: string, type: AttributeTypes) => {
      switch (type) {
        ...
        case AttributeTypes.HTMLString:
          return `"${Util.escapeHTMLString(value)}"`;
  ...
  getAttrStr: (
    ...
    componentInfo: NinComponentInfo
  ): string => {
    attrs
      .keySeq()
      .toArray()
      .map(attr => {
        ...
        if (info == undefined) {
          throw new Error(`this component does not have attr ${attr}`);
        }
