...
export class ModuleFileBuilder {
  ...
  private createActionNames = (file: ComponentFile) => {
    return (
      ...
      `${file.actions
        .map(
          it =>
            `${this.transpiler.createTab(1)}${it!.name} = "${file.name}.${
              it!.name
            }"`
        )
        .join(",\n")}` +
  ...
  private createActions = (file: ComponentFile) => {
    const createActionInterface = (action: NinAction) => {
      return (
        ...
        action.prams
          .map(
            (param, type) =>
              `\n${this.transpiler.createTab(1)}${param}: ${type}`
          )
          .join(",") +
        "\n}\n"
      );
    ...
    const createActionFunc = (action: NinAction) => {
      return (
        `export const ${action.name} = (${action.prams
          .map((param, type) => `${param}: ${type}`)
          .join(", ")}) => ({\n` +
        `${this.transpiler.createTab(1)}type: ActionNames.${name},` +
        action.prams
          .map(it => "\n" + this.transpiler.createTab(1) + it)
          .join(",") +
        "\n});\n"
      );
    };
    return file.actions
      .map(
        action =>
          `${createActionInterface(action!)}\n${createActionFunc(action!)}`
      )
      .join("\n\n");
  };
  private createActionType = (file: ComponentFile) => {
    ...
    return (
      `export type ${file.name}Action =\n` +
      file.actions
        .map(it => `${this.transpiler.createTab(1)}${it!.name}Action`)
        .join(" |\n")
    );
  };
  private createState = (file: ComponentFile) => {
    if (Object.keys(file.store).length === 0) return "";
    return (
      `export interface ${file.name}State {\n` +
      file.store
  ...
  private createInitialState = (file: ComponentFile) => {
    // initialstore??? todo
    return (
      `const initialState: ${file.name}State = {\n` +
      file.state
        .map(it => `${this.transpiler.createTab(1)}${it!.name}: ${it!.initial}`)
        .join(",\n") +
      "\n};\n"
  ...
  private createReducer = (file: ComponentFile) => {
    return (
      `export default function reducer(state: ${
        file.name
      }State = initialState, action: ${file.name}Action): ${
        file.name
      }State {\n` +
      ...
      file.reducer
        .map(
          it =>
            `${this.transpiler.createTab(2)}case ActionNames.${
              it!.actionName
