...
export class TsFileBuilder {
  private static readonly REQUIRED_IMPORT = [`import * as React from "react";`];
  project: Project;
  ...
  private createKeyType(list: List<{ name: string; type: string }>): string {
    return list
      .map(it => `${this.transpiler.createTab(1)}${it!.name}: ${it!.type}`)
      .join("\n");
  }
  ...
  private createClass(file: ComponentFile): string {
    return (
      `export class ${file.name} extends React.Component<Props, ${
        Object.keys(file.state).length !== 0 ? "State" : "{}"
      }> {\n` +
  ...
  private createRender(file: ComponentFile): string {
    let ret = `${this.transpiler.createTab(
      1
    )}render() {\n${this.transpiler.createTab(2)}return (\n`;
    const roots = file.elements
    ...
    const jsx =
      roots.length > 1
        ? `${this.transpiler.createTab(3)}<React.Fragment>\n${roots.join(
            "\n"
          )}\n${this.transpiler.createTab(3)}</React.Fragment>`
        : `${roots.join("\n")}`;
    ret += jsx + "\n";
  ...
  private createJSX(
    ...
    const attrs =
      NodeUtil.getAttrStr(
        node.attributes,
        this.project.getComponentInfo(node.fullName())
      ) +
      (node.classList.size === 0
        ? ""
        : ` className="${node.classList.join(" ")}"`);
    let tag: string = node.type.split(".").pop()!;
    if (node.fullName() === "HTML.textNode") {
      return `${this.transpiler.createTab(tab)}${node.attributes.get("text") ||
        ""}`;
    }
    ...
    return (
      `${this.transpiler.createTab(tab)}<${tag}${attrs}>\n` +
