...
export class Transpiler2 {
  ...
  transpile = async (project: Project, outDir: string) => {
    ...
    project.files.forEach(async (file: ComponentFile) => {
      ...
      await this.writeFile(
        path.join(this.outDir, "src", file.path),
        file.name + ".tsx",
        ts
      );
  ...
  createTab = (num: number): string => {
    const char = this.settings.USE_TAB_CHARACTER
      ? "\t"
      : Array(this.settings.TAB_SIZE + 1).join(" ");
  ...
  private copyTemplate = async (isNoStore: boolean) => {
    ...
    await fs.copy(
      !isNoStore ? "./template/index.tsx" : "./template/index-no-store.tsx",
      path.join(this.outDir, "src", "index.tsx")
    );
    await fs.writeFile(
