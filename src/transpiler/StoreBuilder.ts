...
export class StoreBuilder {
  ...
  private createImport = (modules: Array<{ name: string; path: string }>) => {
    return (
      ...
      modules
        .map(
          it =>
            `import ${it.name}, {${it.name}Action, ${it.name}State} from "./${
              it.name
            }Module"`
        )
        .join("\n")
  ...
  private createCreateStore = (
    ...
    return (
      ...
      `${modules
        .map(it => `${this.transpiler.createTab(2)}${it.name},`)
        .join("\n")}\n` +
  ...
  private createState = (modules: Array<{ name: string; path: string }>) => {
    return (
      ...
      `${this.transpiler.createTab(1)}${modules
        .map(it => `${it.name}: ${it.name}State`)
        .join("\n")}` +
      "\n}"
    );
  };
