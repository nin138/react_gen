...
import { CssValueTypes, default as CssData } from "./Data";
...
export const CssUtil = {
  isValid(attr: string, value: string): CssStatus {
    ...
    switch (type) {
      case CssValueTypes.Int:
        flag = CssUtil.isInt(value);
        break;
      case CssValueTypes.Float:
        flag = CssUtil.isFloat(value);
        break;
      case CssValueTypes.Len:
        flag = CssUtil.isLen(value);
        break;
      case CssValueTypes.Color:
        flag = CssUtil.isColor(value);
        break;
      case CssValueTypes.ZeroToOne:
        flag = CssUtil.isZeroToOne(value);
        break;
      case CssValueTypes.Merge:
        flag = CssUtil.isValidMergedAtr(attr, value);
        break;
      case CssValueTypes.Other:
        flag = CssUtil.isOtherAttrValid(attr, value);
        break;
    }
  ...
  isInt(v: string): boolean {
    return isFinite(+v) && !/[.]/.test(v);
  },
  isFloat(v: string): boolean {
    return isFinite(+v);
  },
  isZeroToOne: (v: string): boolean => isFinite(+v) && +v >= 0 && +v <= 1,
  ...
  isOtherAttrValid: (attr: string, v: string): boolean => {
    let f = false;
    CssData.values.get(attr).values!!.forEach((data: string) => {
      if (data == v) f = true;
    });
    return f;
  },
