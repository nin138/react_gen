...
export class CssClassManager {
  ...
  private copy(...differ: object[]): CssClassManager {
    return Object.assign(
      Object.create(CssClassManager.prototype),
      this,
      ...differ
    );
  }
  ...
  updateAttr(name: string, attr: string, value: string) {
    return this.copy({
      classList: this.classList.update(name, v => v.set(attr, value))
    });
  }
  getCss(name: string): Css | undefined {
    return this.classList.get(name);
  }
  getAllClassName(): string[] {
    return this.classList.keySeq().toArray();
  }
  getCssString(): string {
