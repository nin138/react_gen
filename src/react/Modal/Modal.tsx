...
export default class Modal extends React.Component<Props> {
  ...
  render() {
    ...
    const inputs = (this.props.input)?this.props.input.map(it => <div className="c-modal__main__input-wrap">{it.name}:<input className="c-modal__main__input--text" key={it.name} type="text" ref={it.name}/></div>) : "";
    const buttons = (this.props.buttons)? this.props.buttons
        .map(it => <button className="c-modal__main__button" key={it.name} onClick={() => this.onItemClick.bind(this, it.listener)()}>{it.name}</button>) : "";
    return(<section className="c-modal">
      <div className="c-modal__main">
        ...
        <div className="c-modal__main__input-area">
          {inputs}
        </div>
        <div className="c-modal__main__button-wrap">{buttons}</div>
        <div className="c-modal__close-btn" onClick={e => this.props.actions.close()}>✖</div>
      </div>
    </section>)
  }
}
