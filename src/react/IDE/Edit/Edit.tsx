...
export default class Edit extends React.Component<Props, {}> {
  private createTab() {
    return (
      <React.Fragment>
        <li
          className={`c-edit__head__tab-area__item ${
            this.props.selectedTab === EditTabs.Attributes
              ? "c-edit__head__tab-area__item--selected"
              : ""
          }`}
        ...
        {this.props.selectedItemId !== ROOT_ID &&
        this.props.project.getComponentInfo(this.getActiveElement().fullName())
          .hasCss === true ? (
          <li
            className={`c-edit__head__tab-area__item ${
              this.props.selectedTab === EditTabs.CSS
                ? "c-edit__head__tab-area__item--selected"
                : ""
            }`}
  ...
  private createBody() {
    ...
    const tab =
      !initializer.hasCss && this.props.selectedTab === EditTabs.CSS
        ? EditTabs.Attributes
      ...
      case EditTabs.Attributes:
        return (
          <AttributeEditor
            ...
            element={element}
            changeAttribute={(id, attr, value) =>
              this.props.actions.tree.changeAttribute(id, attr, value)
            }
          />
        );
      case EditTabs.CSS:
        return (
          <CssEditor
            ...
            cssClassManager={this.props.cssClassManager}
          />
        );
    }
  }
  ...
  private getActiveElement(): NinElement {
    return this.props.nodes.get(this.props.selectedItemId);
  }
  render() {
    return (
      <section className="c-edit">
        <div className="c-edit__head">
          <h1>edit</h1>
          <ul className="c-edit__head__tab-area">{this.createTab()}</ul>
        </div>
        <div className="c-edit__body">{this.createBody()}</div>
      </section>
    );
  }
}
