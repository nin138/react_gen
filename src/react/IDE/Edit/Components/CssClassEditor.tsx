...
export default class CssClassEditor extends React.Component<Props> {
  render() {
    ...
    const nodes = attrs
      ...
      .toArray()
      .sort(
        (a, b) => (CssUtil.getAttrOrder(a) < CssUtil.getAttrOrder(b) ? -1 : 1)
      )
      .map(v => {
        return (
          <CssAttr
            ...
            changeCss={(value: string) =>
              this.props.changeCss(this.props.className, v, value)
            }
          />
        );
      });
    return (
      <div>
        <div className="c-CssClassEditor__head">
          ...
          <button
            className="CssClassEditor__head__del"
            onClick={() => this.props.removeClass(this.props.className)}
          >
            ×
          </button>
        </div>
      ...
      case CssValueTypes.Color:
        return (
          <input
            ...
            onChange={e => this.props.changeCss(e.target.value)}
            type="color"
          />
        );
