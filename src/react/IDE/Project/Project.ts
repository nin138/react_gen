...
export class Project {
  ...
  constructor(index: SavedIndex, files: SavedFile[], css: SavedCss) {
    ...
    const tmpFiles: List<ComponentFile> = List(
      files.map(it => new ComponentFile(`${it.path}.${it.name}`))
    );
    ...
    files.forEach(it => {
      ...
      it.node.forEach(node => {
        const info: NinComponentInfo =
          NodeUtil.getPathFromFullName(node.type) === HTML_PATH
            ? HTML_TAGS.find(it => `${it!.path}.${it!.type}` === node.type)!
            : tmpFiles
                .find(it => it!.fullName() == node.type)
  ...
  getComponentInfo(fullName: string): NinComponentInfo {
    ...
    const type = path.pop();
    if (path.join(".") == HTML_PATH) {
      return HTML_TAGS.find(it => it!.type === type)!;
    }
  ...
  addFile(fullName: string, elements = Map<string, NinElement>()): Project {
    return this.copy({
      files: this.files.set(fullName, new ComponentFile(fullName, elements))
    });
  }
  ...
  addNode(element: NinElement): Project {
    return this.copy({
      files: this.files.update(this.activeFile, it => it.addNode(element))
    });
  }
  moveNode(moveNodeId: string, parentId: string, ref: string | null): Project {
    return this.copy({
      files: this.files.update(this.activeFile, it =>
        it.moveNode(moveNodeId, parentId, ref)
      )
    });
  }
  removeNode(id: string): Project {
    return this.copy({
      files: this.files.update(this.activeFile, it => it.removeNode(id))
    });
  }
  addCssClassToNode(id: string, className: string): Project {
    return this.copy({
      files: this.files.update(this.activeFile, it =>
        it.addCssClassToNode(id, className)
      )
    });
  }
  removeCssClassFromNode(id: string, className: string): Project {
    return this.copy({
      files: this.files.update(this.activeFile, it =>
        it.removeCssClassFromNode(id, className)
      )
    });
  }
  changeNodeAttribute(id: string, attr: string, value: string): Project {
    return this.copy({
      files: this.files.update(this.activeFile, it =>
        it.changeAttribute(id, attr, value)
      )
    });
  }
  rebuildTree(
  ...
  componentize(id: string, componentName: string): Project {
    const nodes = this.files
      .get(this.activeFile)
      .copyNodes(id)
      .update(id, it => it.changeParent(ROOT_ID));
    const ret = this.addFile("components." + componentName, nodes).removeNode(
      id
    );
    const parent = this.files.get(this.activeFile).elements.get(id).parent;
    return ret.addNode(
      new NinElement(
        ret.files.get("components." + componentName).getComponentInitializer(),
        parent
      )
    );
  }
    ...
    const getFileHtml = (file: ComponentFile): string => {
      const getNodeHtml = (
        ...
        const children = node.children
          .map(it => getNodeHtml(nodes.get(it!), nodes))
          .join("");
        const attrString =
          NodeUtil.getAttrStr(
            node.attributes,
            this.getComponentInfo(node.fullName())
          ) +
          (node.classList.size != 0)
      ...
      return file.elements
        .filter(it => it!.parent == ROOT_ID)
        .map(it => getNodeHtml(it!, file.elements))
        .join("");
    };
  ...
  changeCssValue(className: string, attr: string, value: string): Project {
    return this.copy({
      cssManager: this.cssManager.updateAttr(className, attr, value)
    });
  }
  private copy(...differ: object[]): Project {
  ...
  private getHTMLRow(tag: string): string {
    ...
    return initializer.hasChild
      ? `<${tag}${NinComponentString.Attributes}>${
          NinComponentString.Children
        }</${tag}>`
      : `<${tag}${NinComponentString.Attributes}/>`;
  }
}
