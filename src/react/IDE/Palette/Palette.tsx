...
export default class Palette extends React.Component<Props, State> {
  ...
  onDragStart(e: React.DragEvent<any>, v: string) {
    ...
    e.dataTransfer.setData(
      "data",
      JSON.stringify(this.props.project.getComponentInfo(v))
    );
  }
  render() {
    const tabs = (
      <div className="c-palette__tab">
        {Array.from(new Set(Object.keys(PaletteTab))).map((it: any) => (
          <div
            className={`c-palette__tab__item${
              PaletteTab[this.state.tab] == it
                ? " c-palette__tab__item--selected"
                : ""
            }`}
            onClick={() => {
    ...
    const components = list
      .filter(it => it.toLowerCase().includes(this.state.search.toLowerCase())) // to do sort
    ...
    return (
      <section className="c-palette">
        <div className="c-palette__head">
          <h1>Palette</h1>
          <input
            className="c-palette__head__input"
            type="text"
        ...
        {tabs}
        <div className="c-palette__list">{components}</div>
      </section>
    );
  }
}
